...
export const SIGNAL_INSTRUMENTS = ['Dollar', 'EURUSD', 'GBPUSD'];
export const SIGNAL_LABELS      = { Dollar: 'Dollar', EURUSD: 'EUR/USD', GBPUSD: 'GBP/USD' };

export function getSignalDisplay(signals) {
  const arr   = Array.isArray(signals) ? signals : [];
  const score = arr.length;
  const max   = SIGNAL_INSTRUMENTS.length;
  const color = score === max ? '#00e676' : score === 2 ? '#4cd964' : score === 1 ? '#ffa502' : 'var(--text-muted)';
  ...
  return `<div style="display:flex;align-items:center;gap:3px" title="${title}">${dots}<span style="font-size:11px;font-weight:700;color:${color};margin-left:5px">${score}/${max}</span></div>`;
}
